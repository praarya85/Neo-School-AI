...
def extract_text(file):
    try:
        ...
        for page in reader.pages:
            # extract_text() in pypdf is robust for Unicode/Hindi
...
def run_ai_audit(exam_text, syllabus_text):
    ...
    Note: If the text contains Hindi (Devanagari), analyze it carefully and provide feedback in standard Hindi.
    ...
    if st.button("🚀 INITIATE DEEP AUDIT"):
        with st.status("Neo is analyzing documents...", expanded=True) as status:
            ...
            if e_text and s_text:
                ...
                st.error("Could not extract text. Ensure PDFs are digital, not scans.")
